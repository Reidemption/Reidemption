...
def main():
    global TotalAge
    hasht = hashtable.HashTable(30000)
    time1 = time.time()
    with open("InsertNames.txt","r") as f, open("DeleteNames.txt", "r") as delete, open("RetrieveNames.txt", "r") as retrieve:
        for line in f:
            ...
            s2 = hasht.Insert(s)
            if not s2:
                print(f"Error: {words[2]} already registered. (insert)")
        ...
        for line in delete:
            ...
            s2 = hasht.Delete(s)
            if not s2:
                print(f"Error: {ssn} not registered. (delete)")
        ...
        for line in retrieve:
            ...
            s2 = hasht.Retrieve(s)
            if not s2:
                print(f"Error: {ssn} never registered (retrieve)")
            else:
                age += int(s2.age)
                count += 1
